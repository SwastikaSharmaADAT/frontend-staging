...
import selectArrow from '../../../public/assets/language-select-arrow.png'
...
const dropdownIndicatorStyles = (base) => {
  let changes = {
    background: `url(${selectArrow}) no-repeat center center`,
    width: '30px',
    color: '#fff',
  }
...
const LanguageSelect = () => {
  ...
  const formatOptionLabel = ({ label, langCode }) => (
    <div style={{ display: 'flex' }}>
      <img width="20px" height="15px" src={createFlagUrl(langCode)} alt={`${langCode} language flag`} />
      <div>{label}</div>
    </div>
  )
