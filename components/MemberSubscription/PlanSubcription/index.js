...
import TermsModalSection from '../../ArtistSubscription/TermsModalSection'
import VendorPaymentModal from '../../ArtistSubscription/VendorPaymentModal'
...
const PlanSubcription = ({ userCountry, isArtist }) => {
  ...
  const selectPlan = (title, subscription1) => {
    if (subscription) {
      if (subscription1.title === subscription.title && purchasedSubscription.subscriptionPeriod === proPricing) {
    ...
    setTermsModal(true)
    if (title !== 'plus') {
      setBillingPeriod(title === 'pro' ? proPricing : '')
    }
    if (title !== 'plus') {
  ...
  const buyPlan = async () => {
    ...
    const resultAction = await dispatch(
      subscriptionCheckout({
        data:
          selectSubscription.title === 'pro'
            ? {
                planId: selectSubscription._id,
                planType: proPricing,
                langCode: langCode ? langCode : 'en',
              }
            : { planId: selectSubscription._id, langCode: langCode ? langCode : 'en' },
        t,
      })
    )
    const result = await unwrapResult(resultAction)
  ...
  const { t } = useTranslation(['errorResponses', 'subscriptions', 'successResponses', 'translation'])
  ...
  const getMonthlyPrice = (pricing) => {
    if (proPricing === 'monthly') {
      return pricing.monthly
    } else if (proPricing === 'quarterly') {
      return parseFloat(pricing.quarterly) / 3
    } else if (proPricing === 'halfYearly') {
      return parseFloat(pricing.halfYearly) / 6
    } else if (proPricing === 'yearly') {
        ...
        return (
          <>
            <Price>
              ...
              {
                formatPrice(
                  decimalSeparatorMethod((getMonthlyPrice(item.price) * conversionRate).toFixed(2), decimalSeparator)
                )[0]
              }
              <span>
            ...
            <Select name="proPricingSelect" value={proPricing} onChange={(e) => setProPricing(e.target.value)}>
                  ...
                  <SectionColsSm className="InnerSmWrap" key={item._id}>
                    <InnerSectionColsTop>
                      ...
                      <MiddleHeadWrap className={classesObj[item.title] && classesObj[item.title].light}>
                        ...
                        {/* <BottomDiv>{ReactHtmlParser(item.description)}</BottomDiv> */}
                      </MiddleHeadWrap>
                      <BottomColsBar
                        className={[
