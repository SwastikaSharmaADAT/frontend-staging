...
const SectionContent = (props) => {
  ...
  return (
    <>
      <SectionContentWrap>
        {props.userData && props.userData.description && !isEditingPublications ? (
        ...
        {isEditingPublications ? (
          <DescriptionBox className={props.descValue > 1000 && "errorBoder"}>
            <SectionDespTextarea
              value={props.descValue}
              onChange={props.descOnChange}
              //maxLength={1000}
              placeholder={t(`publication.placeholderDescription`)}
            ></SectionDespTextarea>
          </DescriptionBox>
        ) : null}
        {
          isEditingPublications && (<CountDiv className={props.descCharCount > 1000 ? "errorRd countDiv" : "countDiv"}>
                              {props.descCharCount}
                              {t(`publication.characterLimit`)}
                            </CountDiv>)
        }
        {
          isEditingPublications && props.descCharCount > 1000 && <div className="errorRd errorDiv">{t(`about.errorAbout`)}</div>
        }
        <ImagesSection
          description={props.userData && props.userData.description}
          images={props.userData && props.userData.images}
          publicationsImages={props.publicationsImages}
...
SectionContent.propTypes = {
  ...
  publicationsImages: PropTypes.array,
