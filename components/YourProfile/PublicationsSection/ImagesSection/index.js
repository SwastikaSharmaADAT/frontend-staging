...
const ImagesSection = (props) => {
  ...
  return (
    <>
      {!isEditingPublications && props.images && props.images.length > 0 ? (
        <ImagesSectionWrap>
      ...
      ) : props.images && props.images.length === 0 && !props.description && !isEditingPublications ? (
        <div className="NoWrap"> {t(`publication.noPublicationsFound`)}</div>
      ) : props.images && props.images.length === 0 && props.description && !isEditingPublications ? (
        <div className="NoWrap"></div>
      ) : null}
      {isEditingPublications ? (
        <ImagesSectionWrap>
          {props.publicationsImages && props.publicationsImages.length > 0 ? (
            <>
              {props.publicationsImages.map((img, index) => (
                <ImgWrap key={index}>
                  <img
                    src={createResizeImageUrl(img.pictureUrl, 'auto', 320, 'mediaLibrary')}
                    onError={(e) => {
          ...
          {props.publicationsImages && props.publicationsImages.length < 5 ? (
            <ImgWrap className="LastChild" onClick={() => props.openModal()}>
              <IoCameraSharp />
              {t(`publication.addImages`)}
            </ImgWrap>
          ) : null}
        </ImagesSectionWrap>
      ) : null}
