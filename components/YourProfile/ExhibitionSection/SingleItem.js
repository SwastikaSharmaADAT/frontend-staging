...
function SingleItem({ article }) {
  ...
  return (
    <SectionArticles key={article._id} onClick={() => openInNewTab(`/exhibition/${article.articleSlug}`)}>
      <SectionImg>
        <img
          src={
      ...
      {article && article.title && <ArticleHeading>{title ? title : article && article.title}</ArticleHeading>}
      {article && article.userId && <ArticleAuthor>{name ? name : nameOfUser}</ArticleAuthor>}
    </SectionArticles>
  )
}
