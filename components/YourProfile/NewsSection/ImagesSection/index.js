...
const ImagesSection = (props) => {
  ...
  return (
    <>
      {!isEditingNews && props.images && props.images.length > 0 ? (
        <ImagesSectionWrap>
      ...
      ) : props.images && props.images.length === 0 && !props.description && !isEditingNews ? (
        <div className="NoWrap">{t(`profile:news.noExhibitionsFound`)}</div>
      ) : props.images && props.images.length === 0 && props.description && !isEditingNews ? (
        <div className="NoWrap"></div>
      ) : null}
      {isEditingNews ? (
        <ImagesSectionWrap>
          {props.newsImages && props.newsImages.length > 0 ? (
            <>
              {props.newsImages.map((img, index) => (
                <ImgWrap key={index}>
                  <img
                    src={createResizeImageUrl(img.pictureUrl, 'auto', 320, 'mediaLibrary')}
                    onError={(e) => {
          ...
          {props.newsImages && props.newsImages.length < 5 ? (
            <ImgWrap className="LastChild" onClick={() => props.openModal()}>
              <IoCameraSharp />
              {t(`profile:news.addImages`)}
            </ImgWrap>
          ) : null}
        </ImagesSectionWrap>
      ) : null}
