            ...
            <FollowsNameWrap>
              ...
              <FollowersCount>
                ...
                {follower.followersCount > 1
                  ? t(`followersPopup.followerPlural`)
                  : t(`followersPopup.follower`)}
              </FollowersCount>
            </FollowsNameWrap>
          </FollowsWrap>
          {userData.userRoleType === 'personal' && accountType === 'personal' && (
            <>
              {follower.followed && userData.userIdentity === 'verifiedUser' ? (
                <FollowedButton>{t(`followersPopup.followed`)}</FollowedButton>
              ) : follower.followed && userData.userIdentity === 'authUser' ? (
                <FollowButton onClick={() => followUnfollowUser(follower._id, 'unfollow', 'followers')}>
                  {t(`followersPopup.unfollow`)}
                </FollowButton>
              ) : !follower.followed && follower.username === loggedInUsername ? null : (
                <FollowButton onClick={() => followUnfollowUser(follower._id, 'follow', 'followers')}>
                  {t(`followersPopup.follow`)}
                </FollowButton>
              )}
            </>
          )}
