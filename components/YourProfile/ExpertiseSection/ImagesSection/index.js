...
const ImagesSection = (props) => {
  ...
  return (
    <>
      {!isEditingExpertise && props.images && props.images.length > 0 ? (
        <ImagesSectionWrap>
      ...
      ) : props.images && props.images.length === 0 && !props.description && !isEditingExpertise ? (
        <div className="NoWrap">{t(`expertise.noExpertiseFound`)}</div>
      ) : props.images && props.images.length === 0 && props.description && !isEditingExpertise ? (
        <div className="NoWrap"></div>
      ) : null}
      {isEditingExpertise ? (
        <ImagesSectionWrap>
          {props.expertiseImages && props.expertiseImages.length > 0 ? (
            <>
              {props.expertiseImages.map((img, index) => (
                <ImgWrap key={index}>
                  <img
                    src={createResizeImageUrl(img.pictureUrl, 'auto', 320, 'mediaLibrary')}
                    onError={(e) => {
          ...
          {props.expertiseImages && props.expertiseImages.length < 5 ? (
            <ImgWrap className="LastChild" onClick={() => props.openModal()}>
              <IoCameraSharp />
              {t(`expertise.addImages`)}
            </ImgWrap>
          ) : null}
        </ImagesSectionWrap>
      ) : null}
