...
`
const InviteSection = () => {
  const { t } = useTranslation(['profile','translation','successResponses', 'giftcard'])
  ...
  return (
    <>
      <InviteContainer>
        <SectionHeading>{t(`giftcard:title`)}</SectionHeading>
        <p>{t(`giftcard:myGiftCardContent.makeCard`)}</p>
        <InviteButton onClick={() => router.push('/giftcard')}>{t(`invite:seeMoreButtonLabel`)}</InviteButton>
      </InviteContainer>
    </>
  )
}
