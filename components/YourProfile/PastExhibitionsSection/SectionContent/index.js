...
const SectionContent = (props) => {
  ...
  return (
    <>
      <SectionContentWrap>
        {props.userData && props.userData.description && !isEditingPastExhibitions ? (
        ...
        {isEditingPastExhibitions ? (
          <DescriptionBox className={props.descCharCount > 1000 && "errorBoder"}>
            <SectionDespTextarea
              ...
              //maxLength={1000}
              placeholder={t(`pastExhibition.placeholderDescription`)}
            ></SectionDespTextarea>
          </DescriptionBox>
        ) : null}
        {
          isEditingPastExhibitions && (<CountDiv className={props.descCharCount > 1000 ? "errorRd countDiv" : "countDiv"}>
                              {props.descCharCount}
                              {t(`pastExhibition.characterLimit`)}
                            </CountDiv>)
        }
        {
          isEditingPastExhibitions && props.descCharCount > 1000 && <div className="errorRd errorDiv">{t(`about.errorAbout`)}</div>
        }
        <ImagesSection
          description={props.userData && props.userData.description}
          images={props.userData && props.userData.images}
          pastExhibitionsImages={props.pastExhibitionsImages}
