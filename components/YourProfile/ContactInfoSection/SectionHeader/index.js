...
import { setEditingContact } from '../../../../modules/profile/myProfileSlice'
          ...
          {props.userData.userIdentity === 'verifiedUser' &&
            (!isEditingContact ? (
              <TopButtons onClick={() => startEditing()}>
                <IoPencil /> {t(`contact.edit`)}
              </TopButtons>
            ) : (
              <>
                <TopButtons onClick={() => cancelEditing()}>{t(`contact.cancel`)}</TopButtons>
                <TopButtons className="primary" onClick={() => stopEditing()}>{t(`contact.save`)}</TopButtons>
              </>
            ))}
        </SectionHeadButtons>
      </SectionHead>
