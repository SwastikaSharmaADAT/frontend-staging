...
const ImagesSection = (props) => {
  ...
  return (
    <>
      {!isEditingAbout && props.images && props.images.length > 0 ? (
        <ImagesSectionWrap>
      ...
      ) : props.images && props.images.length === 0 && !props.description && !isEditingAbout ? (
        <div className="NoWrap">{t(`about.noAboutText`)}</div>
      ) : props.images && props.images.length === 0 && props.description && !isEditingAbout ? (
      ...
      {isEditingAbout ? (
        <ImagesSectionWrap>
          {props.aboutImages && props.aboutImages.length > 0 ? (
            <>
              {props.aboutImages.map((img, index) => (
                <ImgWrap key={index}>
                  <img
                    src={createResizeImageUrl(img.pictureUrl, 'auto', 320, 'mediaLibrary')}
                    onError={(e) => {
          ...
          {props.aboutImages && props.aboutImages.length < 5 ? (
