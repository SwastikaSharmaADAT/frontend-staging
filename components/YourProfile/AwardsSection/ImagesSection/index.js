...
const ImagesSection = (props) => {
  ...
  return (
    <>
      {!isEditingAwards && props.images && props.images.length > 0 ? (
        <ImagesSectionWrap>
      ...
      ) : props.images && props.images.length === 0 && !props.description && !isEditingAwards ? (
        <div className="NoWrap">{t(`award.noAwardFound`)}</div>
      ) : props.images && props.images.length === 0 && props.description && !isEditingAwards ? (
        <div className="NoWrap"></div>
      ) : null}
      {isEditingAwards ? (
        <ImagesSectionWrap>
          {props.awardsImages && props.awardsImages.length > 0 ? (
            <>
              {props.awardsImages.map((img, index) => (
                <ImgWrap key={index}>
                  <img
                    src={createResizeImageUrl(img.pictureUrl, 'auto', 320, 'mediaLibrary')}
                    onError={(e) => {
          ...
          {props.awardsImages && props.awardsImages.length < 5 ? (
