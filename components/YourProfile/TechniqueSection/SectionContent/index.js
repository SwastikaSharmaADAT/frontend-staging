...
const SectionContent = (props) => {
  ...
  return (
    <>
      <SectionContentWrap>
        {props.userData && props.userData.description && !isEditingTechnique ? (
        ...
        {isEditingTechnique ? (
          <DescriptionBox className={props.descCharCount > 1000 && "errorBoder"}>
            <SectionDespTextarea
              ...
              //maxLength={1000}
              placeholder={t(`technique.placeholderDescription`)}
            ></SectionDespTextarea>
            {/* <span className="countDiv">
              {props.descCharCount}
              {t(`technique.characterLimit`)}
            </span> */}
          </DescriptionBox>
        ) : null}
        {
          isEditingTechnique && (<CountDiv className={props.descCharCount > 1000 ? "errorRd countDiv" : "countDiv"}>
                              {props.descCharCount}
                              {t(`about.characterLimit`)}
                            </CountDiv>)
        }
        {
          isEditingTechnique && props.descCharCount > 1000 && <div className="errorRd errorDiv">{t(`about.errorAbout`)}</div>
        }
        <ImagesSection
          description={props.userData && props.userData.description}
          images={props.userData && props.userData.images}
          techniqueImages={props.techniqueImages}
...
SectionContent.propTypes = {
  ...
  techniqueImages: PropTypes.array,
