...
`
function ConnectionRequestSentItem({ item, redirectToUserProfile, deleteConnectionRequest }) {
    ...
    <FollowingList key={item._id}>
      <FollowsWrap>
        ...
        <FollowsNameWrap>
          ...
          <FollowersCount>
            {item.followersCount} {item.followersCount > 1 ? t(`followers`) : t(`follower`)}
          </FollowersCount>
        </FollowsNameWrap>
      </FollowsWrap>
      <DisconnectButton
        disabled={item.isRequestDeleted ? true : false}
        onClick={() => deleteConnectionRequest(item.username, item._id)}
      >
        {item.isRequestDeleted ? t(`requestDeletd`) : t(`deleteRequest`)}
      </DisconnectButton>
    </FollowingList>
  )
}
ConnectionRequestSentItem.propTypes = {
  item: PropTypes.object,
  redirectToUserProfile: PropTypes.func,
  deleteConnectionRequest: PropTypes.func,
}
export default ConnectionRequestSentItem
