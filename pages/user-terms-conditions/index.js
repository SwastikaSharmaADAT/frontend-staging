...
const UserTermsConditions = () => {
  ...
  return (
    <>
      <Head>
        <title>User's Terms and Conditions | ARTMO Social Network</title>
        <meta name="description" content="User's Terms and Conditions | ARTMO overview, content and eligibility, fees and payments, termination and account cancellation" />
      </Head>
      <FeedWrapper>
        <YourProfileContainer className={appLanguageCode === 'ar' ? 'rtl-ar-content' : ''}>
          <HeadingSection userContent={userContent} />
          <SectionContent userContent={userContent} />
          <ButtonWrapper>
            <Button onClick={() => router.push('/privacy-policy')}>
              {t(`usersTermsConditions.seePrivacyPolicy`)}
            </Button>
            <Button onClick={() => router.push('/vendor-terms-conditions')}>
              {t(`usersTermsConditions.seeVendorTermsConditions`)}
            </Button>
          </ButtonWrapper>
...
export default publicRoute(UserTermsConditions)
